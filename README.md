...
4. Chọn bảng mã utf8mb4\_unicode\_ci rồi bấm **Tạo** (Create).
...
4. Tìm đoạn cấu hình dưới đây và sửa lại thông tin cho đúng:  
   ...
   DB\_HOST=127.0.0.1  
   DB\_PORT=3306  
   DB\_DATABASE=mengroom    
   DB\_USERNAME=root        
   DB\_PASSWORD=          

   *(Lưu ý: DB\_DATABASE là tên database bạn vừa tạo, DB\_USERNAME mặc định là root, DB\_PASSWORD mặc định để trống).*  
5. Lưu file lại.
...
3. Thêm dữ liệu mẫu (Sản phẩm, User ảo...):  
   php artisan db:seed
...
* Cửa sổ 1 (Chạy Backend):  
  ...
  *Màn hình sẽ hiện thông báo: Server running on \[http://127.0.0.1:8000\]*  
* Cửa sổ 2 (Chạy Frontend):  
  Gõ lệnh:  
  npm run dev

  *Màn hình sẽ hiện chữ VITE v.v...*
